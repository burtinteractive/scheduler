...
var datepickr = (function() {
	...
	var currDay = thedate.getUTCDate();
	var datepickrs = [],
			...
			if(target[0]) {
				switch(target[0]) {
					...
					case 'day':
						...
						//this sets the new date format. Need to send it to html
						$('#'+elements[1]).val(formatDate(new Date(this.currentYearView, this.currentMonthView, e.target.innerHTML).getTime(), this.config));
						this.close();
					break;
				}
			}
	...
	function buildNode(nodeName, attributes, content, functions) {
		var element;
		...
		if(functions != null) {
			for(var function1 in functions) {
				
			//	console.log(functions[function1]);
				//element[attribute] = attributes[attribute];
				//element.onclick = functions[function1];
				element.setAttribute('onclick', attributes[attribute]);
			}
		}
		return element;
	}
	...
	function buildDays(firstOfMonth, numDays, currentMonthView, currentYearView) {
		...
		for(i = 1; i <= numDays; i++) {
			...
			if(currentMonthView < currMonth){
				row.appendChild(buildNode('td',{ className: 'none_sel'}, buildNode('span', { className: 'day', id:i+'day'+(currentMonthView+1) }, i)));

			}else{
				//now check if it's current month and day is later
				if(currMonth == currentMonthView){
					
					if( i < currDay){
		
						row.appendChild(buildNode('td', { className: 'none_sel'}, buildNode('span', { className: 'none_sel', id:i+'day'+(currentMonthView+1), title:'test' }, i)));
					
					}else{
						
						row.appendChild(buildNode('td', todayClassName, buildNode('span', { className: 'day', id:i+'day'+(currentMonthView+1), title:'test', onClick:'showDate(this);highlight(this)' }, i, {onclick:'showDate(this)',onclick:'highlight(this)'})));
					}
				}else{
	...
	function buildCalendar() {
		...
		var calendar = buildNode('table', { className: 'main_cal' }, buildNode('thead', null, buildNode('tr', { className: 'weekdays' }, buildWeekdays(this.config.weekdays))));
	...
	return function(elementId, userConfig) {
		var self = this;
		//var self = $("#"+elementId);
		//split the id so I can add more elements to append stuff to.
		elements = elementId.split(" ");
		//this.element = document.getElementById(elementId);
		this.element = document.getElementById(elements[0]);
		...
		this.open = function(e) {
			addEvent(document, 'onload', self.documentClick);
		...
		//$(this.element).appendTo(this.calendar);
		
				$("#"+$(this.element).attr('id')).append(this.calendar);
				$("#"+$(this.element).attr('id')).append("<button class='next' id='next2'>Next</button>");
